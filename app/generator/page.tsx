...
const DIMENSION_OPTIONS: ScoreDimension[] = [
  'functionality',
  'ux',
  'pricing',
  'integration',
...
export default function GeneratorPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      <Link
        href="/"
        className="inline-flex items-center gap-2 px-4 py-2 mb-6 text-sm font-medium text-gray-600 dark:text-gray-400 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm hover:border-blue-300 dark:hover:border-blue-700 hover:text-blue-600 dark:hover:text-blue-400 transition-all"
      >
      ...
      <div className="text-center mb-10">
        ...
        <p className="text-gray-500 dark:text-gray-400 max-w-xl mx-auto">
          Select your use case and priority dimensions. Our system will recommend the Top 3 tools
          based on the six-dimension scoring model.
        </p>
      </div>
      ...
      <section className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-6 sm:p-8 mb-8">
        <div className="mb-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {scenarioOptions.map((opt) => (
              ...
              >
                <div
                  className={`text-sm font-bold mb-1 ${scenario === opt.value ? 'text-blue-700 dark:text-blue-400' : 'text-gray-700 dark:text-gray-300'}`}
                >
                  {opt.label}
                </div>
        ...
        <div className="mb-8">
          <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
            2. Priority Dimensions (Multi-select)
          </label>
          <p className="text-xs text-gray-400 dark:text-gray-500 mb-3">
            Selected dimensions get higher weight in recommendations
          </p>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {DIMENSION_OPTIONS.map((dim) => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-5 h-5 flex-shrink-0 flex items-center justify-center rounded-md border-2 transition-all ${isSelected ? 'bg-indigo-500 border-indigo-500' : 'border-gray-300 dark:border-gray-600'}`}
                  >
                    {isSelected && <Check className="w-3.5 h-3.5 text-white" />}
                  </div>
                  <div className="text-left min-w-0">
                    <div
                      className={`text-sm font-semibold ${isSelected ? 'text-indigo-700 dark:text-indigo-400' : 'text-gray-700 dark:text-gray-300'}`}
                    >
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          ...
          {results && (
            <button
              onClick={handleReset}
              className="inline-flex items-center gap-2 px-6 py-4 text-gray-500 dark:text-gray-400 font-medium hover:text-gray-700 dark:hover:text-gray-300 transition-colors"
            >
              Reset
            </button>
          )}
        ...
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                Based on "{SCENARIO_LABELS[scenario]}" scenario
                {priorityDims.length > 0 &&
                  `, focusing on ${priorityDims.map((d) => DIMENSION_LABELS[d]).join(', ')}`}
              </p>
            </div>
            <button
              onClick={handleShare}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {results.map((result) => (
              <div
                key={result.tool.id}
                className={`relative bg-white dark:bg-gray-900 rounded-2xl border-2 shadow-sm overflow-hidden transition-all ${result.rank === 1 ? 'border-amber-300 dark:border-amber-600 shadow-amber-500/10' : result.rank === 2 ? 'border-gray-300 dark:border-gray-700' : 'border-orange-200 dark:border-orange-800'}`}
              >
                <div
                  className={`absolute top-0 right-0 w-12 h-12 flex items-center justify-center text-white font-extrabold text-lg ${result.rank === 1 ? 'bg-gradient-to-br from-amber-400 to-amber-600' : result.rank === 2 ? 'bg-gradient-to-br from-gray-400 to-gray-600' : 'bg-gradient-to-br from-orange-400 to-orange-600'}`}
                  ...
                  <div className="flex items-start gap-4 mb-4 pr-8">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1.5">
                        <span
                          className={`px-2 py-0.5 rounded-md text-xs font-bold ${GRADE_STYLES[result.scoreResult.grade]}`}
                        >
                          {result.scoreResult.grade} Grade
                        </span>
                        <span className="text-xs text-gray-400 dark:text-gray-500">
                          Overall {result.scoreResult.total.toFixed(1)}/10
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  {result.matchedDimensions.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {result.matchedDimensions.map((dim) => (
                        <span
                          key={dim}
                          className="px-2 py-0.5 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 rounded-md text-xs font-medium"
                        >
                          ✓ {DIMENSION_LABELS[dim]}
                        </span>
                      ))}
