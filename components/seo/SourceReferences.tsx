  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-900 rounded-xl p-5 border border-gray-100 dark:border-gray-800">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-base font-bold text-gray-900 dark:text-white">Sources & References</h3>
      </div>

      <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
        This review was conducted using our {toolName ? `${toolName} ` : ''}6-dimension evaluation
        framework. We verify all claims against primary sources and update reviews regularly.
      </p>
      ...
      <ul className="space-y-3">
        {displaySources.map((source, index) => (
          <li key={index} className="flex items-start gap-3">
            <span
              className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-semibold flex-shrink-0 mt-0.5 ${TYPE_COLORS[source.type || 'documentation']}`}
            >
      ...
      {showLastUpdated && (
        <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
          <p className="text-xs text-gray-500 dark:text-gray-400">
            <strong>Last updated:</strong> {updateDate} · Reviews are updated every 90 days or when
            major product changes occur.
          </p>
        </div>
      )}
    </div>
